...
**Hawksnest** · **Magpie**). One repo, two deliverables:

- **`android/`** — the Dragonfly app (Kotlin/Compose): launcher/dashboard for the sibling apps,
  in-app APK update channel (GitHub Releases, SHA-256 verified, `PackageInstaller` flow), a live
  **suite-status dashboard** (+ a home-screen Glance widget), a narrated **weekly digest**, shared
  suite settings, and the suite's **config broker** — a signature-permission ContentProvider
  siblings read their server URLs from.
...
`version.json` on push to `main`, and phones update through Dragonfly. Every app in the suite shares
one signing key, which is what makes the broker's signature permission and in-place updates work.
